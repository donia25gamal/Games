                        ...
                        <form className='my-4' onSubmit={submitForm}>
                            <div className="row">
                                <div className="col-md-6 mb-4">
                                    ...
                                    <small className='text-danger error-form text-white'>{joiErrors.filter((err) => err.context.label === 'first_name')[0]?.message}</small>
                                </div>
                                <div className="col-md-6 mb-4">
                                    <input onChange={getUSerData} name='last_name' id='last_name' type="text" placeholder='Last Name' className='form-control' />
                                    <small className='text-danger error-form text-white'>{joiErrors.filter((err) => err.context.label === 'last_name')[0]?.message}</small>
                                </div>
                            </div>
                            <div className="mb-4">
                                <input onChange={getUSerData} name='email' id='email' type="text" placeholder='Email Address' className='form-control' />
                                <small className='text-danger error-form text-white'>{joiErrors.filter((err) => err.context.label === 'email')[0]?.message}</small>
                            </div>
                            <div className="mb-4">
                                <input onChange={getUSerData} name='age' id='age' type="number" placeholder='age' className='form-control ' />
                                <small className='text-danger error-form text-white'>{joiErrors.filter((err) => err.context.label === 'age')[0]?.message}</small>
                            </div>
                            <div className="mb-4">
                                <input onChange={getUSerData} name='password' id='password' type="password" placeholder='Password' className='form-control' />
                                <small className='text-danger error-form text-white'>{joiErrors.filter((err) => err.context.label === 'password')[0]?'Invalid Password : must be at least 8 characters':null}</small>
                            </div>
