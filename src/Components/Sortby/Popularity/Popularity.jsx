...
export default function Popularity() {
    ...
    const [popularity, setPopularityGames] = useState([]);
    ...
    return <>
        <div className="container text-center">
            <div className="row">
                {popularity.length > 0 ? popularity.slice(0, count).map((game, index) =>
                        ...
                        <div onClick={() => { getDetails(game.id) }} title={game.platform === "PC (Windows)" ? 'Avaliable on Windows' : 'Avaliable on Browser'} className='shadow game-card'>
                            ...
                            <div className="p-3 game-card-body">
                                <div className="d-flex justify-content-between align-items-center">
                                    ...
                                    <h6 className='text-white bg-danger free p-2'>FREE</h6>
                                </div>
                                <p className='text-truncate text-muted'>{game.short_description}</p>
                                <div className="d-flex justify-content-between align-items-center">
                                    <i className="fa-solid fa-square-plus text-white-50"></i>
                                    <div>
                                        <span className='category px-2 py-1 fw-bold rounded-3 me-2'>{game.genre}</span>
            ...
            {count > popularity.length ? null : <button onClick={moreGames} className='btn btn-outline-secondary'>More Games</button>}
        </div>
    </>
}
