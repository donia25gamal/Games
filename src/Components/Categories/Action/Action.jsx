...
export default function Action() {
    ...
    return <>
        <div className="container text-center">
            <div className="row">
                {action.length > 0 ? action.slice(0, count).map((game, index) =>
            ...
            {count > action.length ? null : <button onClick={moreGames} className='btn btn-outline-secondary'>More Games</button>}
        </div>
    </>
}